    ...
    */
    ...
    public function index(Request $request)
    {
        ...
        if(!Session::has('user_log')){
            // $userInfo = file_get_contents('http://ip-api.com/json/'.$request->ip());
            $userData['user_id'] = Auth::user()->id;
            $userData['ip'] = $request->ip();
            $userData['user_info'] = serialize($userInfo);
            UserLog::create($userData);
            Session::put('user_log','ok');
        }
        ...
        if($user = \Auth::user()){
            ...
            if(in_array($segment_1, $all_ap)){
                if(!in_array($segment_1, $access_permissions)){
