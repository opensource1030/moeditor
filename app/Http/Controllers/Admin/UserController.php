...
class UserController extends Controller {
    ...
    public function index() {
        ...
        $find_by_email = trim(Input::get('search_email'));
        if (Auth::user()->hasRole('admin')) {
            ...
            $user = Auth::user();
            if($owner = $user->owner->first()){
                $data['users'] = User::with('profile', 'campaigns', 'role', 'owner')->whereHas('owner', function ( $query ) use ($owner) {
                    $query->where('owner_id', '=', $owner->id);
                });
            } else {
                $data['users'] = User::with('profile', 'campaigns', 'role', 'owner')->whereHas('owner', function ( $query ) use ($user) {
        ...
        if (!empty($find_by_email)) {
            $data['users'] = $data['users']
                                ->where('email', 'like', '%' . $find_by_email . '%')
                                ->orWhere('name','like', '%' . $find_by_email . '%');
        }
    ...
    public function add() {
        ...
        // if (Auth::user()->can('add_user')) {
            ...
            $data['user']->access_permissions = array();
            ...
            $data['access_permissions'] = DB::table('access_permissions')->get();
    ...
    {
        $find_by_email = trim(Input::get('search_email'));
        $owners = UserOwner::select('user_id')->where('owner_id',$id)->get()->toArray();
        $owner = array_pluck($owners,'user_id');
        array_push($owner,$id);
        $users = UserLog::with('users')->whereIn('user_id',$owner)->orderBy('created_at','DESC');
        if(!empty($find_by_email)){
            $users = $users->whereHas('users', function ( $query ) use ($find_by_email) {
                $query->where('email', 'like', '%' . $find_by_email . '%');

            });
        }
        $users = $users->paginate(10);
        return view('admin.users.logs',compact('users','id'));
    }
    ...
    public function edit($uid) {
        ...
        // if (Auth::user()->can('add_user')) {
            ...
            if ($user) {
                ...
                if ( $user_owner > 0 || Auth::user()->hasRole('admin')) {
                    ...
                    $data['roles'] = Role::get()->lists('role_title', 'id');
                    $data['user'] = $user;
                    $data['user']->access_permissions = unserialize($user->access_permissions);
	...
	public function upsert() {
        ...
        if ($validator->fails()) {
            ...
            if ($uid > 0) {
                ...
                $user->role_id = \Input::get('role_id');
                $user->access_permissions = serialize(\Input::get('access_permissions'));
                $user->save();
                ...
                if (!$user) {
                    $user = User::create([
                                ...
                                'access_permissions' => serialize(\Input::get('access_permissions')),
                            ]);
                    UserProfile::create([
                        'user_id' => $user->id,
                    ...
                    if (Auth::user()->hasRole('advertiser')) {
                        ...
                        if($owner = Auth::user()->owner->first()){
                            $uo           = new UserOwner();
                            $uo->owner_id = $owner->id;
                            $uo->user_id  = $user->id;
                            $uo->save();
                        }
                    }
                    ...
                    return redirect()->back()->withInput()->withError('Email Already Registered');
                }
            }
