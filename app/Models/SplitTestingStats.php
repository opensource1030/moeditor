...
class SplitTestingStats extends Eloquent {
    ...
    public static function record_event($campaign_id, $template_id, $event, $label, $name, $value, $ip=null)
    {
        ...
        if( $event == 'page_open' ) {
            ...
            $stat = new static;
            $stat->id_address = $request_id;
            $stat->event = 'ao_' . $event;
            $stat->label = 'ao_' . $label;
            $stat->name = 'ao_' . $name;
            $stat->value = $value;
            $stat->campaign_id = intval($campaign_id);
            $stat->is_ajax = \Request::ajax();
...
//        }
        ...
        $splt = static::select('event', 'id_address', 'campaign_id', 'template', 'label', 'created_at')->whereIn('campaign_id', $campaignIdArray);
...
//            }
        ...
        foreach ($splt as $spl) {
            ...
            if(!isset($unique[$spl->campaign_id][$spl->template])){
                $unique[$spl->campaign_id][$spl->template] = [];
            }

            if (!isset($unique[$spl->campaign_id][$spl->template][md5($spl->id_address . date('y-m-d', strtotime($spl->created_at)))])) {
                $unique[$spl->campaign_id][$spl->template][md5($spl->id_address . date('y-m-d', strtotime($spl->created_at)))] = 1;
            }
            ...
            if ($spl->event == 'navigate') {
                if (isset($return[$spl->campaign_id][$spl->template]['total_clicks'])) {
                    $return[$spl->campaign_id][$spl->template]['total_clicks'] += 1;
                } else {
                    $return[$spl->campaign_id][$spl->template]['total_clicks'] = 1;
                }
                // if ($spl->label == 'mailto') {
            ...
            if ($spl->event == 'optin') {
                if (isset($return[$spl->campaign_id][$spl->template]['total_optins'])) {
                    $return[$spl->campaign_id][$spl->template]['total_optins'] += 1;
                } else {
                    $return[$spl->campaign_id][$spl->template]['total_optins'] = 1;
                }
            }
            if ($spl->event == 'pixel') {
                if (isset($return[$spl->campaign_id][$spl->template]['total_pixel'])) {
                    $return[$spl->campaign_id][$spl->template]['total_pixel'] += 1;
                } else {
                    $return[$spl->campaign_id][$spl->template]['total_pixel'] = 1;
                }
            }
        ...
        foreach ($unique as $campaign => $templates) {
            foreach($templates as $template => $items)
            {
                $return[$campaign][$template ]['total_unique'] = count($items);
            }
        }
...
//            Cache::store( 'memcached' )->put( 'agmbci_' . join( '_', $campaignIdArray ).'_'.$from.'_'.$to, $return, $expiresAt );
    ...
    public static function detailedCampaignStats($campaign_id, $from = '', $to = '', $start = 0, $length = 10, $order = [ 0 => [ 'column' => 4, 'dir' => 'desc']]) {
        //prevent memory limit status code 500 errors
        //halabuda@gmail.com
...
//            $graph_raw[ $record->template ][ strtotime( date( 'Y-m-d', strtotime( $record->created_at ) ) ) ] = 0;
        ...
        foreach ($all_data as $record) {
            ...
            if($row['event'] != 'Default'){
                ...
                if($record->event == 'navigate')
                {
                    $return['summary'][$record->template]['total_clicks'] += 1;
                }
                ...
                if ($return['summary'][$record->template]['total_unique'] > 0)
                {
                    $return['summary'][$record->template]['click_percent'] = round((($return['summary'][$record->template]['total_clicks'] * 100) / $return['summary'][$record->template]['total_unique']), 1) . '%';

                    $return['summary'][$record->template]['optin_percent'] = round((($return['summary'][$record->template]['total_optin'] * 100) / $return['summary'][$record->template]['total_unique']), 1) . '%';

                    $return['summary'][$record->template]['pixel_percent'] = round((($return['summary'][$record->template]['total_pixel'] * 100) / $return['summary'][$record->template]['total_unique']), 1) . '%';
                }
